...
Both this and the [ER diagram](docs/erd.svg) are regenerated by a script rather
than screenshotted, so they can't drift out of date:
...
The lineage is derived straight from `target/manifest.json`. The ER diagram's
layout is curated, but the script cross-checks the foreign keys it draws against
the model's `relationships` tests and fails if they diverge.
